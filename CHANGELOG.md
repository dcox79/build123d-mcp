...
- **`render_view()` no longer dereferences an unusable face triangulation.** The
  bounded tessellation worker extracts OCC meshes per face, reports the indexes of
  faces whose triangulation is missing or empty, and renders the remaining healthy
  faces when possible. If every face is unmeshable, the existing skipped-shape
  path now returns an actionable triangulation diagnostic instead of a raw
  `None.NbNodes` error or a silent empty render.
...
- **The exact mesh gate silently welded a genuinely off-vertex tessellation node into place instead of reporting it.** Cross-validating our own gate's PASS verdict against CADGenBench's own official mesh sanity checker (run directly from its source, not the packager's known-broken `--official-sanity-check` path) turned up a submission the gate passed cleanly — `0 mesh non-manifold edges, exact-subprocess, 0 open edges` — that the official checker rejected with `"vertex #492: an edge endpoint node is 1.993e-01 mm from the vertex (> deflection 0.005717647362146476 mm); edge polygon orientation is inconsistent with FirstVertex/LastVertex"`. The gate's vertex-merge step (closing fillet/cone/pole apices where an edge's tessellated endpoint should coincide with its BREP vertex) unioned every candidate node into the vertex unconditionally, with no check that the node was actually anywhere near it — a patched/healed face (a sliver sew, a tolerance-fudged repair patch) whose polygon endpoint missed its vertex by a fraction of a millimetre got silently welded into "the same point" instead of flagged. Traced to the exact location: two ~1.5mm² and ~5mm² BSpline sliver faces at a healed junction — the geometric signature of a repair patch, not the original import or the edit itself. Added the same guard CADGenBench's own checker uses (confirmed by reading its source directly): before unioning a candidate node into a BREP vertex, verify it is within the mesh deflection of the vertex's analytic position; report each vertex that fails as a new `mesh_vertex_deflection_defects` count (`_mesh_defects_exact`'s return tuple gains a 6th element) rather than merging it silently. Wired into `_gate_report`'s pass/fail decision and `reasons`, threaded through the out-of-process mesh-gate subprocess (`_gate_subprocess.py`) so `export()` on a large part sees it too, and added to `locate_gate_defects()` as a new `mesh_vertex_deflection_defect` kind with the vertex's coordinates and the measured deviation. Verified against the real submission that exposed the bug: the fix reproduces CADGenBench's own verdict exactly (2 defective vertices located, one of them the reported vertex #492, `passes_gate` now correctly `False`) with no regressions across the existing exact-mesh-check test suite (clean solids, fillets, a sphere's poles/seams, disjoint touching bodies — all still pass with 0 false positives).
- **Follow-up fixes from an adversarial review of the above.** (1) `_mesh_defects_exact`'s return-type annotation was never updated from the old 5-tuple, breaking `mypy` in CI with 15 errors across every return site and caller — fixed. (2) The guard above was only applied to the base non-manifold pass; the open-edge deflection ladder's own, independent vertex-merge (`_open_pass`, escalated to finer deflections when a shape is genuinely open at the base rung) still unioned unconditionally, so a defect invisible at the base deflection but exposed only at a finer rung could still slip through silently — the same guard now runs there too, propagated back through `_open_ladder()`. (3) `locate_gate_defects()`'s new `mesh_vertex_deflection_defect` locator had no size/time budget (unlike its gate-side counterpart) and re-tessellated the shape from scratch with different parameters than the other locators' shared pass moments earlier — added a triangle-count ceiling (raising into the existing `locator_error` path rather than risking the whole subprocess's timeout) and confirmed the gate's own coarser tessellation parameters are the correct ones to keep, not a shared-tessellation optimization that turned out to make the common case worse (measured directly: matching the finer angular tolerance the other locators use balloons one real part's triangle count 14x). (4) That locator also used Euclidean distance where the gate (and CADGenBench's real algorithm, confirmed by reading its source) uses max-per-axis — fixed to match, since the mismatch meant `locate_gate_defects()` could point at a vertex the authoritative gate does not actually fail on. (5) It also rebuilt edge→face adjacency via the slow ancestor-list pattern `_edge_face_adjacency` was specifically written to avoid — switched to reuse that helper directly. (6) Added positive-case tests for both the gate-side and locator-side checks (previously only asserted 0 on clean shapes) — real malformed geometry that reproduces this defect class is a repair-patch artifact that resists synthetic construction (sewing a mismatched boundary heals it geometrically; hand-editing a triangulation node is discarded on the next mesh pass), so the tests drive the exact `BRep_Tool.Pnt_s` lookup the real bug hit via a targeted monkeypatch on an otherwise-clean `Box`, including one that isolates the ladder-specific guard from (2) by calibrating the deviation to be invisible to the base pass and visible only once the ladder escalates. (7) Added the new FAIL-reason text to the `b123d-repair` skill's Step 0 diagnose table and gave Rung 5 a matching recipe (re-patch/re-sew at a tighter tolerance, not the drop-the-face technique its existing unmeshable-face case uses) — the table's own contract is "the class picks the recipe," and this class had neither an entry nor a recipe.
- **A second, fresh adversarial review round on the fixes above found the ladder guard from (2) still undercounted, plus 3 smaller gaps — all fixed.** The merge `max(vertex_defl_defects, 1) if _ov_vdefl else vertex_defl_defects` laundered the ladder's own finding into a boolean before folding it in, so if the ladder caught defects the base pass didn't, the reported total silently capped at +1 regardless of how many *distinct* vertices were actually involved (proven empirically: two independently off-vertex vertices, both only visible via the ladder, reported `vdefl==1` instead of `2`). Replaced the boolean with a set of each defective vertex's own world-space position, unioned by identity between the base pass and every ladder rung that ran — an exact count now, with a new regression test for the multi-vertex undercount and another confirming the same vertex found by both passes still dedupes to 1, not 2. Also: the new `_open_pass` vertex guard had no `_open_deadline` check, unlike the edge-adjacency loop right above it in the same function — added, matching that established pattern. `server.py`'s MCP-exposed `locate_gate_defects` docstring (the actual schema text served to LLM clients) was never updated with the new `mesh_vertex_deflection_defect` kind even though the internal `tools/locate.py` copy was — synced. And the SKILL.md addition from the first follow-up round contradicted itself — "Do not drop this face" followed two sentences later by "re-sew (option 4/5)," where option 4 is literally "Drop + tolerant sew" — narrowed to option 5 only. Two deeper points from this same review round (a possible gate/locator count mismatch when the base pass finds nothing but the ladder does; whether the ladder's sticky-OR-across-rungs semantics can diverge from CADGenBench's own discard-on-success retry logic) are real but require more careful, dedicated design than a quick fix — tracked as follow-up issues rather than rushed here.
...
- **`b123d-modeling` skill: a better fix for extending a boss or relocating a planar/annular face.** The skill already warned that additive edits with exactly-coincident faces often fail to fuse; extending a boss, or moving a bore's counterbore opening or a shoulder along its own axis — all a constant cross-section at the join — are the most common real instances of that trap, and "interpenetrate slightly" is not the best available fix for any of them. Field evidence from two independent CADGenBench editing fixtures: (1) a hollow boss needing to reach a target height, where an agent burned five different bridging/overlap fuse variants — all failing with the same non-manifold or two-disjoint-solids error — before abandoning that boss for an easier target elsewhere in the part; (2) raising an annular bore opening by 10mm, where the same class of construction failed three times in a row (an overlap version, a no-overlap version, and a version with the exported defect patched directly) — every attempt passing `validate()` in memory but failing the stricter `export()` round-trip check — before the agent switched technique and succeeded immediately. The actual fix, already validated in independent past sessions but never written into the shipped skill: extrude the feature's *own* face instead of adding a separately-built primitive. A face extruded from the part's own boundary shares the exact underlying geometry with what it's fused against, so the boolean dissolves the shared boundary cleanly — a coincidentally-matching new primitive does not, even when perfectly positioned. Added both the one-sided technique (`BRepFeat_MakePrism` on the face to move, for "extend/relocate toward a target on one end," the more common case) and the symmetric technique (`extrude()` each end-cap face by half the growth, then fuse, no raw OCCT needed, for a boss growing equally on both ends). An xhigh-effort adversarial review of this same change (before merge) caught that the first draft only tested a single Z-aligned, single-end case and had three real bugs a broader test would have needed to catch: the one-sided snippet hardcoded the extrusion direction to global +Z instead of deriving it from the seat face's own normal, silently collapsing any off-axis boss (tilted, horizontal, on a cast body) to zero volume with `IsDone()` still `True`; the symmetric snippet's "repeat for the other end" instruction fused the second extension onto the original, un-extended `part` instead of the already-extended result, silently discarding the first end's growth; and it extruded each end by the *full* requested growth instead of half despite the prose saying otherwise, doubling the final length. All three fixed and re-verified: the one-sided technique against both a Z-aligned and an X-aligned boss, and the symmetric technique against a boss extended on both ends in one worked example (not a "repeat this line" instruction) — exact volume and total-growth match in every case, `BRepCheck_Analyzer` valid, hole preserved. The one-sided snippet also gained an explicit `IsDone()` guard before touching `.Shape()`, matching the defensive-OCCT convention `b123d-repair` established for raw-OCCT builders one release earlier.
